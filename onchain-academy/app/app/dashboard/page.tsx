            ...
            <header className="relative z-10 border-b border-white/[0.06]">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    ...
                    <nav className="flex items-center gap-1">
                        <Link href="/courses" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-zinc-500 hover:text-white hover:bg-white/5 transition-all font-bold">
                            <BookOpen className="w-3.5 h-3.5" /> Courses
                        </Link>
                        <Link href="/leaderboard" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-zinc-500 hover:text-white hover:bg-white/5 transition-all font-bold">
                            <BarChart3 className="w-3.5 h-3.5" /> Leaderboard
                        </Link>
                        <Link href="/profile" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-zinc-500 hover:text-white hover:bg-white/5 transition-all font-bold">
                            <User className="w-3.5 h-3.5" /> Profile
                        </Link>
                        <Link href="/certificates/cert-001" className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs text-zinc-500 hover:text-white hover:bg-white/5 transition-all font-bold">
                            <Award className="w-3.5 h-3.5" /> Certs
                        </Link>
                        <div className="w-px h-5 bg-white/[0.06] mx-1" />
                        <Link href="/settings" className="p-2 rounded-lg hover:bg-white/5 transition-colors text-zinc-500 hover:text-white">
            ...
            <main className="relative z-10 max-w-7xl mx-auto px-6 py-10">
                ...
                >
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {[
                            { slug: "intro-to-solana", title: "Intro to Solana", desc: "Learn the fundamentals of Solana blockchain", xp: 200, progress: 0 },
                            { slug: "smart-contracts-101", title: "Smart Contracts 101", desc: "Build your first Anchor program", xp: 500, progress: 0 },
                            { slug: "defi-on-solana", title: "DeFi on Solana", desc: "Understand SPL tokens and AMMs", xp: 750, progress: 0 },
                        ].map((course, i) => (
                            <Link
                                key={i}
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-[10px] text-zinc-600 font-bold flex items-center gap-0.5">Start <ChevronRight className="w-3 h-3" /></span>
                                </div>
                ...
                >
                    <Link href="/profile" className="flex items-center gap-4 p-6 rounded-xl bg-white/[0.02] border border-white/[0.06] hover:border-white/[0.12] transition-all group">
                        {user.avatar ? (
                        ...
                        <div className="flex-1">
                            <div className="text-sm font-bold text-white group-hover:text-neon-green transition-colors">{user.name || user.username || "Builder"}</div>
                            <div className="text-[10px] text-zinc-500">{user.email || `@${user.username}`}</div>
                        </div>
                        <ChevronRight className="w-4 h-4 text-zinc-600 group-hover:text-neon-green transition-colors" />
                    </Link>
                </motion.div>
            </main>
        </div>
